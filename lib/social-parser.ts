...
const BUILDING_PREFIXES = ["Torre", "Residencias", "Edificio", "Bloque", "Conjunto", "Centro", "Parque"]

/** Extract a building/complex name like "Torre Las Américas" or "Edificio Central". */
function detectBuilding(text: string): string {
  for (const prefix of BUILDING_PREFIXES) {
    const re = new RegExp(`${prefix}\\s+[A-ZÁÉÍÓÚÑ][\\wáéíóúñA-Z\\s]{1,30}`, "u")
    const m = text.match(re)
    if (m) return m[0].trim().replace(/\s{2,}/g, " ").slice(0, 60)
  }
  return ""
}
...
export function parsePost(text: string): ParseResult {
  ...
  if (!trimmed) return { name: "", location: "", phone: "", building: "", signalType: "unclear" }
  ...
  return {
    ...
    building: detectBuilding(trimmed),
